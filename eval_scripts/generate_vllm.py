...
def main(input_file, output_file, model_path, debug=False, remove_system=True, template='own', temperature=0.6, top_p=1.0, max_tokens=8192, n=1, force_generate=True, add_think_before_answer=False, add_oat_evaluate=False, any_true=False, skip_scoring=False, output_eval=None, no_split_think=False):
    ...
    if force_generate or (not os.path.exists(dec_output_path)):
        ...
        outputs, skipped_indices = generate_vllm(messages, model_path, template=template, temperature=temperature, top_p=top_p, max_tokens=max_tokens, n=n)
...
def generate_vllm(messages, model_path, template='own', temperature=0.6, top_p=0.95, max_tokens=8192, n=1):
    ...
    for i in range(len(messages)):
        ...
        # Try per-sample generation and skip on backend errors (e.g., block-table mismatch)
        try:
            out = llm.generate([gen_prompt], sampling_params)
            # vLLM returns a list even for single prompt
            outputs_per_sample.append(out[0])
        except Exception as e:
            err_msg = str(e)
            print(f"[WARN] skipping sample {i} due to generation error: {err_msg}")
            skipped_indices.append(i)
            continue

    return outputs_per_sample, skipped_indices
