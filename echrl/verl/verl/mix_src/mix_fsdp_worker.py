...
class MIXActorRolloutRefWorker(Worker):
    ...
    def generate_sequences(self, prompts: DataProto):
        ...
        if self._is_offload_param:
            # NOTE: the grad is already in CPU, only offload param here
            offload_fsdp_param_and_grad(module=self.actor_module_fsdp, offload_grad=self._is_offload_grad)
        # clear kv cache
        torch.cuda.empty_cache()
    ...
    def compute_log_prob(self, data: DataProto):
        ...
        collect_entropy = os.getenv('COLLECT_ENTROPY', '0') == '1'
        if not hasattr(self, '_entropy_debug_printed'):
            print(f"[DEBUG][fsdp_worker] collect_entropy={collect_entropy}, ENV={os.getenv('COLLECT_ENTROPY')}")
        ...
        with self.ulysses_sharding_manager:
            ...
            if collect_entropy:
                old_log_probs, entropy = result
                data.batch['old_log_probs'] = old_log_probs
                data.batch['entropy'] = entropy
            else:
                old_log_probs = result
                data.batch['old_log_probs'] = old_log_probs
            data = self.ulysses_sharding_manager.postprocess_data(data)

        if collect_entropy:
            output = data.select(batch_keys=['old_log_probs', 'entropy'])
        else:
