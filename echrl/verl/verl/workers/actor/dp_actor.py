...
class DataParallelPPOActor(BasePPOActor):
    ...
    def compute_log_prob(self, data: DataProto) -> torch.Tensor:
        ...
        # Handle dynamic bsz reordering
        if use_dynamic_bsz:
            indices_flat = list(itertools.chain.from_iterable(indices))
            assert len(indices_flat) == log_probs.size(0), f"{len(indices_flat)} vs. {log_probs.size()}"
        ...
        if collect_entropy:
            ...
            if use_dynamic_bsz:
                # Use the same revert_indices as log_probs
                assert len(indices_flat) == entropy.size(0), f"{len(indices_flat)} vs. {entropy.size()}"
