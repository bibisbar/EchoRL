...
class RayPPOTrainer(object):
    ...
    def _save_checkpoint(self):
        ...
        # Additionally dump a Hugging Face checkpoint for each saved step, matching the "best" save format.
        # This allows step checkpoints to be consumed by downstream tooling that expects HF layout.
        try:
            self.actor_rollout_wg.save_checkpoint_hf(actor_local_path)
        except AttributeError:
            # Backward compatibility: older worker implementations may not provide HF saver.
            print("[WARN] actor_rollout_wg.save_checkpoint_hf not available; skipping HF-format save for step checkpoint.")
